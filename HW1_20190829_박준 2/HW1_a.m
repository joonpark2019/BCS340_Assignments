...
initial_current = 0;
%iterate through all of the values of I_test, and stop at the current which
%first leads to a non-zero current
for i_input = I_test
...
for i_n=1:length(I_inputs)
    ...
    %Check for rate:
    if abs(rate - 20) < 0.5
        disp("Closest Rate found to 20 Hz [Hz]:");
        disp(rate);
        disp("Current for 20Hz firing rate [mA]:");
        disp(I_inputs(i_n));
...
%sptimes: a num_trials x spk_train length sized array with times for the
%spikes
