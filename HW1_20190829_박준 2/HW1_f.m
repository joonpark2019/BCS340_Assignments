...
% function for plotting the response function
% inputs:
% --> n_trials: number of spike trains used to measure rate for a single
% input current
% --> I_noise: noise current input [mA], noise is samples from a uniform
...
function response = response_curve_conv(n_trials, I_noise, time, min_I, max_I)
    ...
    %iterate through input currents from min_I to max_I and find average
    %rates
