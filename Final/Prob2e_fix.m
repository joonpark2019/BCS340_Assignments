...
thresholds = (mean(comb_rates, "all")-5):0.1:(mean(comb_rates, "all")+5);
desired_r_opt = 0.5*(mu_up + mu_down);
for threshold=thresholds
    p_emp_up = sum(comb_rates > threshold, "all") / length(comb_rates);
    p_emp_down = sum(comb_rates < threshold, "all") / length(comb_rates);
    ...
    r_opt = mean(0.1*find(abs(p_s_up - p_s_down) < 0.05), "all");
    % disp(r_opt);
    if abs(r_opt -  desired_r_opt) < 0.2
        disp(r_opt);
        disp("Threshold!!!!");
        disp(threshold);
    end
